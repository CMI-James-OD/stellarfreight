## Description

Please include a summary of the change and which issue is fixed. Include relevant motivation and context.

Fixes # (issue)

## Type of change

- [ ] Bug fix (non-breaking change which fixes an issue)
- [ ] New feature (non-breaking change which adds functionality)
- [ ] Breaking change (fix or feature that would cause existing functionality to not work as expected)
- [ ] Documentation update
- [ ] Refactor / code cleanup
- [ ] Smart contract change

## Area affected

- [ ] Frontend (`frontend/`)
- [ ] Backend (`backend/`)
- [ ] Smart Contracts (`contracts/`)
- [ ] CI / Repo config
- [ ] Documentation

## How Has This Been Tested?

Please describe the tests that you ran to verify your changes. Provide instructions so reviewers can reproduce.

- [ ] Frontend: `cd frontend && npm run lint && npm run typecheck && npm test`
- [ ] Contracts: `cd contracts/escrow && cargo fmt --check && cargo clippy -- -D warnings && cargo test`
- [ ] Backend: N/A (backend not yet implemented)

## Checklist

- [ ] My code follows the style guidelines of this project
- [ ] I have performed a self-review of my own code
- [ ] I have commented my code, particularly in hard-to-understand areas
- [ ] I have made corresponding changes to the documentation
- [ ] My changes generate no new warnings
- [ ] I have added tests that prove my fix is effective or that my feature works
- [ ] New and existing tests pass locally with my changes
- [ ] No `.env` files or secrets are included in this PR
- [ ] Any dependent changes have been merged and published in downstream modules
